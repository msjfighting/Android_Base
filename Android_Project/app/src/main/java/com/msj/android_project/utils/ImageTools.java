...
public class ImageTools {
    ...
    public static void fillImg2Pdf (File targetFile, List<File> imgFileList) {
        ...
        try {
            ...
            for (int i = 0; i < imgFileList.size(); i++) {
                ...
                Map<String, Float> sizeMap = getSize(imgFile.getPath(), page, 30, 30);
     ...
     * @return
     */
    private static Map<String, Float> getSize(String imgFile, PDPage page, float paddW, float paddH) {
        Map<String, Float> map = new HashMap<String, Float>();
        Bitmap bufferedImage;
        bufferedImage = BitmapFactory.decodeFile(imgFile);
     ...
     * @return
     */
    private static float resize(String imgFile, float pageW, float pageH){
        Bitmap bufferedImage;
        bufferedImage = BitmapFactory.decodeFile(imgFile);
     ...
     * @return
     */
    private static float resize(Bitmap bufferedImage, float pageW, float pageH) {
        float width = bufferedImage.getWidth();
        float height = bufferedImage.getHeight();
        float scale = 1f;
        // 宽高都小于PDF，原样输出
